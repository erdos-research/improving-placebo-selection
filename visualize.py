...
try:
    ...
    if input("Install `numpy`, `plotly`, `kaleido`, and `Pillow` now? [Y/n]: ").strip().lower(
    ) != "n":
        ...
        for col_idx, col in enumerate(np.array_split(page, n_cols)):
            for item in col[::-1]:
                ...
                fig.add_trace(go.Violin(
                    ...
                    line={
                        "color":
                            gradient_point(np.median(sample_set), gradient=DARKREDGREENGRADIENT)
                    },
...
def model_bias(identifiers: Iterable[str],
    ...
    for id_, x, y in data:
        ...
        delta = sum([
            (x[i] - x[i - 1]) * (y[i] + y[i - 1] - x[i - 1] - x[i]) / 4 for i in range(len(x))
        ])

        fig = go.Figure()
        fig.add_trace(go.Scatter(x=[0, 0], y=[-1, 1], mode="lines", line=dict(color="#e9e9e9")))
        fig.add_trace(
            go.Scatter(x=[-0.5, -0.5], y=[-1, 1], mode="lines", line=dict(color="#f4f4f4")))
        ...
        fig.add_trace(
            go.Scatter(x=[-1, 1], y=[-0.5, -0.5], mode="lines", line=dict(color="#f4f4f4")))
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode="markers+lines",
                marker=dict(color=[
                    gradient_point((b - a) / 2, REDYELLOWGREENGRADIENT) for a, b in zip(x, y)
                ]),
        ...
        fig.add_trace(
            go.Scatter(x=[-1, -1],
                       ...
                       text=[f"<i>ρ</i> = {rho}, <i>MSE</i> = {mse}", f"<i>Δ</i> = {delta:.2%}"],
